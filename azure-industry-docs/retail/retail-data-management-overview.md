...
ms.locfileid: "51654185"
---
# <a name="data-management-in-retail-overview"></a>零售業中的資料管理概觀

## <a name="introduction"></a>簡介

資料是開發和提供更好的零售體驗的基礎。 資料存在於零售組織的各個面向，可用於將整個價值鏈中的見解擷取至運營績效和客戶行為中，並用於改善服務體驗。 從線上瀏覽至社交參與，再到店內購買，存在各種豐富的資料。 不過，擷取資料只是資料管理的一部分。 將不同的資料拼接在一起進行分析，需要正確處理整個組織內的資料，進而改善零售商在經營時做出有影響力之決策的能力。
...
有效的資料使用可以藉由幫助零售商提供更多相關的體驗，來推動客戶購買；例如，零售商可能會向客戶發送通知，內含零售商電子商務網站折扣代碼。  此外，這項資料將提供可操作的見解，公司領導者可以透過以這些資料為基礎的決策，引導他們的行動

提供促銷的動作是由資料點的組合所構成，並由進入商店的客戶觸發。 進行這些連線和產生之動作的能力，是以下列顯示的資料管理模型為依據。

![程序流程](./assets/retail-data-management-assets/process-flow.png)

圖 1

將資料帶入 Azure 時，請考慮資料來源的 3P，及其對零售商希望啟用之方案的適用性。 資料來源的 3P 是購買 (Purchased)、公用 (Public) 及專用 (Proprietary)。

> **購買**資料通常會擴大並強化組織的現有資料，通常包含市場和人口統計資料，可補充組織的資料擷取觸達。 例如，零售商可以購買額外的人口統計資料，以擴大主要的客戶記錄，確保記錄準確且完整。 
>
> **公用**資料是免費提供的，可以從社交媒體、政府資源 (例如地理位置) 及其他線上資源中獲取。 這項資料可以推斷例如與購買模式相關的天氣模式，或表示特定地理位置之間產品流行度之社交參與的見解。 公用資料通常可透過 API 取得。
>
> **專用**資料位於組織內。 它可能是零售商的內部部署系統、SaaS 應用程式，或雲端提供者。 若要存取 SaaS 應用程式提供者中的資料，和其他廠商的資料，API 通常用於溝通廠商系統。 這包括電子商務網站記錄、POS 銷售資料，以及庫存管理系統等資料。

這些不同的資料類型用於來自資料管理管線的各種見解。
...
資料擷取並不限於 Microsoft 技術。 透過 [Azure Marketplace](https://azuremarketplace.microsoft.com?WT.mc_id=retaildm-docs-dastarr)，零售商可能會在 Azure 中設定許多不同的供應商資料庫，以便與現有的內部部署系統搭配使用。 
...
資料成形發生在準備階段的「擷取-轉換-載入 (ETL)」程序中。 資料是從匯入 Azure 的未變更資料來源中擷取，視需要「整理」或重新格式化，並以全新更結構化的格式儲存。 常見的 ETL 資料準備作業是將 .csv 或 Excel 檔案轉換為 parquet 檔案，以便 Apache Spark 等機器學習系統更容易快速讀取和處理。 另一個常見案例是從 .csv 檔案或其他格式建立 XML 檔案或 JSON。 產生的格式更容易與其他分析引擎搭配使用。
