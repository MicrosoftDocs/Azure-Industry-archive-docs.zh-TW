...
若要充分運用您的系統，您必須瞭解模型的工作流程，以及計算與相應放大功能的互動方式。您的軟體可能會有作業、工作或類似的概念。 請運用該知識來設計可以分割工作的項目。 如果在您的模型中有一些自訂的步驟，請將那些步驟設計成允許將輸入分割成較小群組以進行處理。 通常，這被稱為分散-集中模式。
